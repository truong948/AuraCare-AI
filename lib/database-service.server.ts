...
export async function loadAllOrdersServer(): Promise<MockOrder[]> {
  try {
    const supabase = await getSupabaseClient();
    ...
    if (!error && dbOrders && dbOrders.length > 0) {
      return dbOrders.map((ord: any) => ({
        id: ord.id,
        orderNumber: ord.order_number || ord.id.slice(0, 8),
    ...
    console.warn("Failed to load orders from Supabase server side, checking mock fallback:", err);
  }
  return mockOrders;
}
