      ...
      <div className="mx-auto max-w-6xl space-y-6 px-4 sm:px-6 lg:px-8">
        <div className="rounded-[36px] border border-[#dce6df] bg-white p-8 shadow-[0_16px_34px_rgba(15,23,42,0.06)]">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link href="/checkout" className="inline-flex items-center rounded-2xl bg-[#5b8c7a] px-5 py-3 text-sm font-semibold text-white hover:bg-[#4f7c6d]">
              Tiếp đến thanh toán
            </Link>
          </div>
        </div>
        ...
        {items.length === 0 ? (
          <div className="rounded-[32px] border border-dashed border-[#d7e5df] bg-white p-10 text-center">
            ...
            <Link href="/" className="mt-6 inline-flex rounded-2xl bg-[#5b8c7a] px-5 py-3 text-sm font-semibold text-white hover:bg-[#4f7c6d]">
              Quay về cửa hàng
            </Link>
          </div>
        ) : (
          <div className="grid gap-6 xl:grid-cols-[1.4fr_0.6fr]">
            <div className="space-y-4">
              {items.map((item) => {
                ...
                return (
                  <div key={item.productSlug} className="rounded-[28px] border border-[#d7e5df] bg-white p-5 shadow-[0_12px_30px_rgba(15,23,42,0.04)]">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                      ...
                      <div className="flex flex-col gap-3 sm:items-end">
                        ...
                        <button onClick={() => removeItem(product.slug)} className="inline-flex items-center gap-2 text-sm font-semibold text-[#c2410c] hover:text-[#9a3412]">
            ...
            <div className="rounded-[28px] border border-[#d7e5df] bg-white p-5 shadow-[0_12px_30px_rgba(15,23,42,0.04)]">
              ...
              <div className="mt-6 rounded-3xl bg-[#f8fbfa] p-4 text-sm text-[#334155]">
                <p className="font-semibold">Ready for checkout</p>
                <p className="mt-2">Giỏ hàng hiện có {items.length} sản phẩm với tổng giá trị {formatMockPrice(subtotal)}.</p>
              </div>
              <Link href="/checkout" className="mt-6 inline-flex w-full items-center justify-center rounded-2xl bg-[#5b8c7a] px-4 py-3 text-sm font-semibold text-white hover:bg-[#4f7c6d]">
                Đi tới thanh toán
              </Link>
            </div>
          </div>
