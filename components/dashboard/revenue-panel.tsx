...
export function RevenuePanel() {
  ...
  const chartData = useMemo(() => {
    // Chỉ tính doanh thu từ đơn đã giao hoặc đang giao
    ...
    // Nhóm theo ngày - 14 ngày gần nhất
      ...
      <div className="flex flex-col gap-4 rounded-[32px] border border-[#dce6df] bg-white p-6 shadow-sm">
        ...
        <div className="grid gap-4 md:grid-cols-3 mt-4">
          {[
            ...
            <div
              key={label}
              className="group rounded-3xl border border-[#dce6df] bg-gradient-to-br from-teal-50 to-white p-6 transition-all hover:border-[#0d9488] hover:shadow-md"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-medium text-[#5b8c7a]">{label}</p>
